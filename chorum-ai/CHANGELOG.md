...
- **Domain-Aware Extraction (Writing)**: When `domainSignal.primary === 'writing'`, the extraction system now uses writing-specific categories — `character`, `setting`, `plot_thread`, `voice`, `world_rule` — instead of code-centric patterns/decisions/invariants. New types include dedicated decay curves (characters and world rules never decay; plot threads at 90d; voice and setting at 365d) and section headers in the injected context (`## Characters`, `## Active Plot Threads`, `## World Rules`, etc.). Foundation for future domain-specific extractors (research, business, etc.) (`src/lib/learning/analyzer.ts`, `src/lib/learning/types.ts`, `src/lib/chorum/relevance.ts`).
