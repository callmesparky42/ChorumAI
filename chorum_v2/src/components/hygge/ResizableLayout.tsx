 ...
 * Adapted for Chorum v2 Hygge styles
 *
...
export function ResizableLayout({ left, center, right, rightCollapsed = false }: ResizableLayoutProps) {
    ...
    return (
        <div ref={containerRef} className="flex h-full w-full">
            {/* Left Panel */}
            <div
                className="bg-[var(--hg-bg)] overflow-hidden flex-shrink-0 h-full"
                style={{ width: `${leftWidth}%`, display: left ? 'block' : 'none' }}
            >
                {left}
            </div>
            ...
            {left && (
                <div
                    ...
                    className="w-1 bg-[var(--hg-border)] hover:bg-[var(--hg-accent)] transition-colors cursor-col-resize flex-shrink-0"
                />
            )}

            {/* Center Panel */}
            <div
                className="bg-[var(--hg-surface)] overflow-hidden flex-shrink-0 flex flex-col min-h-0"
                style={{ width: left ? `${centerWidth}%` : (rightWidth > 0 ? `${100 - rightWidth}%` : '100%') }}
            >
                {center}
            </div>
            ...
            {/* Right Panel */}
            <div
                className="bg-[var(--hg-bg)] overflow-hidden flex-shrink-0 h-full border-l border-[var(--hg-border)]"
